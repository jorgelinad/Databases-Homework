...
app.post("/products", function (req, res) {
  ...
  pool
    .query(`SELECT * FROM suppliers AS s WHERE s.id=$1`, [newProductSupplier])
    .then((result) => {
...
app.post("/customers/:customerId/orders", function (req, res) {
  ...
  pool
    .query(`SELECT * FROM customers AS c WHERE c.id=$1`, [newCustomerId])
    .then((result) => {
      if (result.rows.length === 0) {
  ...
  pool
    ...
    .then((result) => {
      if (result.rows.length > 0) {
        ...
        return res.status(400).send("The customer not exists!");
      }
    });
});
...
app.delete("/orders/:orderId", function (req, res) {
  ...
  pool.query(`SELECT * FROM orders WHERE id=$1`, [orderId]).then((result) => {
    if (result.rows.length > 0) {
      const query = `DELETE FROM order_items where order_id = ${orderId};
      ...
      return res.status(400).send("The order not exists!");
    }
  });
});
...
app.delete("/customers/:customerId", function (req, res) {
  ...
  pool
    ...
    .then((result) => {
      if (result.rows.length > 0) {
        return res.status(400).send("The customer has one, or more orders!");
      } else {
        pool
